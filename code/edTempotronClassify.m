function [weights, correctRate, CorrectFireRate, CorrectNonFireRate,epoch]= ...
    edTempotronClassify(weights, IsTraining, SimWhichSet, datafolder, MAXEPO, lmd,targetRate,a,b) 
...
if (IsTraining), 
    ...
    if isequal(SimWhichSet,'training set')
        load ([datafolder,'/','PtnCellTrnTst_spk'], 'PtnCellTrn')
        PtnCell = PtnCellTrn;   %训练集cell（1，样本数）
        clear PtnCellTrn;
...
correctRate=zeros(1,maxEpoch);%每一轮的正确率
...
for epoch=1:maxEpoch
    ...
    for iImage = randperm(nImages)
        ...
        nSlices = size(PtnCell{iImage}.AllVec,2);      %AllVec列数即为切片数  2表示返回列
        PtnCell{iImage}.Tgt = false(nNeuronPerOutput*nOutputs,nSlices);  %样本标签
        ...
        for pp = randperm(nSlices)
            ...
            ptn = PtnCell{iImage}.AllVec(:,pp);  %(0+9.*rand(1))*0.1;				% spike timings%改  原没+0.2
            ...
            tgt(lbl*nNeuronPerOutput+1: (lbl+1)*nNeuronPerOutput) = true;     %期望为true的神经元
            PtnCell{iImage}.Tgt(:,pp) = tgt;             %Tgt即保存着改切片的期望值
			
			
			nAfferents = length(ptn);	% number of effective afferent. (e.g. 100)  %转码输入的100个
            ...
            [~, idx_tmp] = sort(P(:,1),'ascend');   %idx_tmp -----1*ones(size(P(:,2))), -1*ones（size（P（:,2））
            P = P(idx_tmp,:,:);
            ...
            for neuron = 1:nOutputs
                ...
                for indNeuronPerOutput = 1:nNeuronPerOutput
					...
					for i = 1:numEvt    %第一轮.第一个样本.第一个切片.第一个神经元.第一个输出通道.第一个事件
						t = P(i,1);
						addr_i = P(i,2);
						c = P(i,3);
                        ...
                        if refractory,
							...
							%	if PtnCell{iImage}.Tgt(1,pp) == true 
							%		Vm_K1 = Vm_K1 + (V0)*weights(addr_i,neuron,indNeuronPerOutput);    %%%%关键！
							%	else
									Vm_K1 = Vm_K1 + (V0)*weights(addr_i,neuron,indNeuronPerOutput)+0;%(0+1.*rand(1))*0.03;    %%%%关键！
							%	end
                            end
                            if ~use_single_exponential,
                    ...
                    PtnCell{iImage}.Out(indTotNeuronOutput, pp) = out;  %indTotNeuronOutput某类中的某个神经元输出，pp为对应切片
