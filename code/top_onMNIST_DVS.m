...
lmd = 5e-2;  %改 1e-2 使用mk时需要调大  %a=0.15 b=0.15时5e-2
% lmd = 5e-2;
% lmd = 2e-2;
% lmd = 1e-2;
% lmd = 1e-3;
...
nNeuronPerOutput = 10;      %改 10 number of neurons at each output channel. (redundancy, majority voting)
...
% filters:
minFS = 3; maxFS = 9; sSFS  = 2;    %change minFS = 3; maxFS = 9; sSFS  = 2;
rot = [0 45 90 135];%change rot = [0 45 90 135];
circularRField   = 1;
...
numTotal = 10000 %100 %10000;     % number of samples totally       %%%%%改10000
ratio_Trn = 0.9 %0.8;

% data folder:
folder0 = ['d_MNISTDVS_sz',num2str(size1), '_',num2str(numTotal), '_trn',num2str(ratio_Trn,'%0.2f'), ...
...
CINsfile = [folder0, '/', 'MNISTDVS.mat'];%MNISTDVS
if ( (reRunAll) || (reGenCINs) || ~exist(CINsfile,'file') )
    sourceMatFile = ['MNIST_DVS_28x28_' num2str(numTotal) '_full.mat']; 
    ...
    [NumCINs]=prepareCINs_MNISTDVS(folder0,sourceMatFile);
end
...
if ( (reRunAll) || (reGenPtnCell) || ~exist(file,'file') )
	...
	tic
    parfor ind = 1:NumCINs   %并行  原1：NumCINs
...
if ( (reRunAll) || (reSplitDataset) || ~exist(file,'file') )
    [PtnCellTrn, PtnCellTst, indTrn, indTst, indNaN] = StableSplit(PtnCell,ratio_Trn,nGroup);  %RandSplit(PtnCell,ratio_Trn,nGroup)
...
%            IsTraining, SimWhichSet, folder0, maxEpoch, lmd,1,a,b);
        [TrainedWt,correctRate, tmp2, tmp3,epoch] = edTempotronClassify_callMEX(weights, ...			% 原
            IsTraining, SimWhichSet, folder0, maxEpoch, lmd,1,a,b);										% 原
    elseif (continueTrnWts)
        timedLog('continue training ..');
        load (file)
%        [TrainedWt,correctRate, tmp2, tmp3,epoch] = edTempotronClassify(TrainedWt, ...
%            IsTraining, SimWhichSet, folder0,maxEpoch, lmd,1,a,b);
        [TrainedWt,correctRate, tmp2, tmp3,epoch] = edTempotronClassify_callMEX(TrainedWt, ...				% 原
            IsTraining, SimWhichSet, folder0,maxEpoch, lmd,1,a,b);											% 原
    end	
...
%			 IsTraining, 'training set',folder0, 1, lmd,1,a,b);
	 	[TrainedWt, RR_trn, RR_trn_TP, RR_trn_TN] = edTempotronClassify_callMEX(TrainedWt, ...   % 原
	 			IsTraining, 'training set',folder0, 1, lmd,1,a,b);								% 原
		 [TrainedWt, RR_tst, RR_tst_TP, RR_tst_TN] = edTempotronClassify(TrainedWt, ...				% 原
			 IsTraining, 'testing set',folder0, 1, lmd,1,a,b);										% 原
%	end
...
if ( (reRunAll) || (reAnalyzeResults) || ~exist(file,'file') )
    [ FinalCorrRate_Trn, FinalCorrRate_Tst, decision_Trn, decision_Tst, freqsCell_Trn, freqsCell_Tst ] = ...   		%改 原[ FinalCorrRate_Trn, FinalCorrRate_Tst, decision_Trn, decision_Tst, freqsCell_Trn, freqsCell_Tst ] = ...
        AnalyzeResults_MajorityVoting(nOutputs, nNeuronPerOutput, folder0);    %改原 AnalyzeResults_MajorityVoting(nOutputs, nNeuronPerOutput, folder0)
    [ CorrRate, decision, freqsCell, ConfMatrix ] = ...   		%改 原[ FinalCorrRate_Trn, FinalCorrRate_Tst, decision_Trn, decision_Tst, freqsCell_Trn, freqsCell_Tst ] = ...
        AnalyzeResults_MajorityVoting_AllTstIn1(nOutputs, nNeuronPerOutput, folder0);
