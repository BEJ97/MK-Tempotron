...
%# codegen
        ...
        for pp = randperm(size(SpkTimings,2)) %AllVec行，2列
            ...
            ptn = SpkTimings(:,pp);    %+0.05;			% spike timings 偏移噪声   0.05*floor(rand(1,1)*2)*2-1
                        ...
                        if refractory,
                            ...
                            if (c ~= -1)    % only for the original input spikes, not the dummy ones.
								if NeuronsTgt(1,pp) == true
									Vm_K1 = Vm_K1 + (V0 - a)*weights(addr_i,neuron,indNeuronPerOutput);    
								else
									Vm_K1 = Vm_K1 + (V0 + b)*weights(addr_i,neuron,indNeuronPerOutput);
								end
                            end
                            if ~use_single_exponential,
                                ...
                                if (c ~= -1)
									if  NeuronsTgt(1,pp) == true
										Vm_K2 = Vm_K2 + (V0 - a)*weights(addr_i,neuron,indNeuronPerOutput);    
									else
										Vm_K2 = Vm_K2 + (V0 + b)*weights(addr_i,neuron,indNeuronPerOutput);
									end
                                end
                                Vm = Vm_K1-Vm_K2;
                            else
