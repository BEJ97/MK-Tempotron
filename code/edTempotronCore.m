...
%# codegen
        ...
        for pp = randperm(size(SpkTimings,2)) %AllVec行，2列
            
            addr= Addresses(:,pp);				% spike address
            ptn = SpkTimings(:,pp);			% spike timings 偏移噪声   0.05*floor(rand(1,1)*2)*2-1
                ...
                for indNeuronPerOutput = 1:nNeuronPerOutput
					...
					for i = 1:numEvt    	
                        ...
                        if ~condition_lastVm_checkup,
							...
							if NeuronsTgt(1,pp) == true                %%%%%%改进  目标为发放
								if(fired == false) 						%%%%并没有发放
									if Vm  >=V_thr    && fired==false 	% fire  改
										fired=true;
										t_fire=t_last;
										out =true;
										if use_single_exponential
							...
							elseif NeuronsTgt(1,pp) == false          %%%%%目标为不发放
								if(fired == true)						%%%实际发放
									if Vm  >=V_thr   && fired==false % fire 改
										fired=true;
										t_fire=t_last;
                        ...
                        if refractory,
                            ...
                            if (c ~= -1)    % only for the original input spikes, not the dummy ones.
								if NeuronsTgt(1,pp) == true
									if(fired == false)
										Vm_K1 = Vm_K1 + (V0 - a)*weights(addr_i,neuron,indNeuronPerOutput);    %%%%改进
									else
										Vm_K1 = Vm_K1 + (V0)*weights(addr_i,neuron,indNeuronPerOutput);        %%%%改进								
									end
								elseif NeuronsTgt(1,pp) == false
									if(fired == true)
										Vm_K1 = Vm_K1 + (V0 + b)*weights(addr_i,neuron,indNeuronPerOutput);    %%%%改进
									else
										Vm_K1 = Vm_K1 + (V0)*weights(addr_i,neuron,indNeuronPerOutput);        %%%%改进
									end
								else
                            ...
                            if ~use_single_exponential,
                                ...
                                if (c ~= -1)
									if  NeuronsTgt(1,pp) == true
										if(fired == false)
											Vm_K2 = Vm_K2 + (V0 - a)*weights(addr_i,neuron,indNeuronPerOutput);     %%%%改进				
										else
											Vm_K2 = Vm_K2 + (V0)*weights(addr_i,neuron,indNeuronPerOutput);%%%%%%%%改进
										end
									elseif NeuronsTgt(1,pp) == false
										if(fired == true)
											Vm_K2 = Vm_K2 + (V0 + b)*weights(addr_i,neuron,indNeuronPerOutput);%%%%%%%%改进
										else
											Vm_K2 = Vm_K2 + (V0)*weights(addr_i,neuron,indNeuronPerOutput);%%%%%%%%改进
										end
									else
