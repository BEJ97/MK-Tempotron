...
for i = 1:1
	...
	% integ switch
	tau_m = 20e6;
	timeslice = 2.0*tau_m;    	% for reconstructing img from event slice Êîπ Âéü1.5*
	...
	CONV_refresh_window_only = 1;  % if 1, only refresh a window. otherwise, refresh whole S1 map. %Êîπ Âéü1

	% Leakage rate (on the fly conv)
	cteloss = 5e-8 %1.25e-8; %1e-8; %5e-8;     % about 1/(20e6),  leakage per nanosecond.   %Êîπ ‘≠5ee-8
	% MAX
	doMAXforEachEvent = 1;      %Êîπ Âéü0
	...
	post_proc_th     = 0.8;%0.2;%0.8; %0.5;  %Êîπ ‘≠0.8 %%%%%%%%%%%%%%%%%%%%%%%∏ƒ£°£°£°£°£°£°£°£°£°£°£°£°‘≠0.8
	...
	numTotal = 131 %100 %10000;     % number of samples totally
	ratio_Trn = 0.9 %0.8;
	...
	CINsfile = [folder0, '/', 'MNISTDVS.mat'];
	if ( (reRunAll) || (reGenCINs) || ~exist(CINsfile,'file') )
		sourceMatFile = ['poker.mat']; 
	...
	if ( (reRunAll) || (reGenPtnCell) || ~exist(file,'file') )
		...
		parfor ind = 1:NumCINs
			...
			[ Timings, V, t ] = MotionSymbolDetector_POKERDVS( CIN, ...
	...
	%% --- Tempotron
	% initialize weights
	nAfferents = size1*size2*(length(minFS:sSFS:maxFS)*length(rot));   %1DÊï∞ÁªÑ32*32*16
	nOutputs = nGroup;  %1DÂÆûÊï∞
	mu_init_wt = 0;
	...
	if ( (reRunAll) || (reInitWeights) || ~exist(file,'file') )
		...
		weights = mu_init_wt + sigma_init_wt * randn(nAfferents, nOutputs, nNeuronPerOutput);       % 1DÊï∞ÁªÑ ÂèëÊîæÊï∞*4(ËæìÂá∫Á±ªÂà´Êï∞)*10 with redundancy 
		save (file, 'weights')
	end
	load (file)
	...
	%             IsTraining, SimWhichSet, folder0, maxEpoch, lmd);
			[TrainedWt,correctRate, tmp2, tmp3,epoch] = edTempotronClassify_callMEX(weights, ...	%ÊîπÂéüÊ≠§ÊñπÊ≥ï
	...
	%             IsTraining, SimWhichSet, folder0,maxEpoch, lmd);
			[TrainedWt,correctRate, tmp2, tmp3,epoch] = edTempotronClassify_callMEX(TrainedWt, ...    %ÊîπÂéüÊ≠§ÊñπÊ≥ï
				IsTraining, SimWhichSet, folder0,maxEpoch, lmd,1,a,b);
		end
