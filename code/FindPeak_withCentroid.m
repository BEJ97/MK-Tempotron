function [ tpeak,centroid_atPeak ]=FindPeak_withCentroid(V,t,centroid,Radius,th)

N = length(V);
indPeak = zeros(1,N);
tpeak = zeros(1,N);
centroid_atPeak = zeros(2,N);
cnt = 0;
...
while i<=N
    ...
    if V(i)>=th
        ...
        if ind==i   % peak
            ...
            cnt = cnt+1; 
            tpeak(cnt) = t(i);
            centroid_atPeak(:,cnt) = centroid(:,i);
            indPeak(cnt) = i;
...
tpeak = tpeak(1:cnt);
centroid_atPeak = centroid_atPeak(:,1:cnt);
