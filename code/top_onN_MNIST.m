...
lmd = 5e-2;  %改 原1e-2 使用mk时需要调大  %a=0.15 b=0.15时5e-2
% lmd = 5e-2;
% lmd = 2e-2;
% lmd = 1e-2;
% lmd = 1e-3;
...
% integ switch
tau_m = 20e6;
timeslice = 0.5*tau_m;    	% for reconstructing img from event slice
...
numTotal = 70000 %100 %10000;     % number of samples totally       %%%%%改原10000
ratio_Trn = 0.9 %0.8;用于训练的百分比

% data folder:
folder0 = ['d_N_MNIST_sz',num2str(size1), '_',num2str(numTotal), '_trn',num2str(ratio_Trn,'%0.2f'), ...
...
CINsfile = [folder0, '/', 'NMNIST.mat'];%N-MNIST
if ( (reRunAll) || (reGenCINs) || ~exist(CINsfile,'file') )
    ...
    [NumCINs]=prepareCINs_N_MNIST(folder0,sourceMatFile);
end
...
if ( (reRunAll) || (reGenPtnCell) || ~exist(file,'file') )
    ...
    for ind = 1:NumCINs   %并行  原1：NumCINs
        ...
        [ Timings, V, t ] = MotionSymbolDetector_NMNIST( CIN, ...
