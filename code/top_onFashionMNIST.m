...
numTrainingImgs = 60000;

nNeuronPerOutput = 10;%5;   % number of neurons at each output channel. (redundancy, majority voting)

d = 0; %0.05;                   % noise density
ratio = 1; %0.90;   % number of pixels that generate spikes / number of '1' pixels
nRepeat = 1;    % repeat CIN (nRepeat) times for each image.
...
% integ switch
tau_m = 20e3;                % PSP kernel time constant
...
cteloss = 1e-5;     % leakage per nanosecond.      
...
% data folder:
folder0 = ['d_Fashion_MNIST_nRpt',num2str(nRepeat), '_4x4filt',str1,maxStr,num2str(post_proc_th),...
    '_wRedun',num2str(nNeuronPerOutput) ,'_cte',mynum2str(cteloss),'_pSliceTAllN'];
...
dt = tau_m/1e3; % dt for lookup table (lut)
tau_s = tau_m/4;
tau1 = tau_m;
tau2 = tau_s;
...
CINsfile = [folder0, '/', 'AER_Fashion_MNISTbRand_500_alt'];
if ( (reRunAll) || (reGenCINs) || ~exist(CINsfile,'file') )
    run_fashion_img2aer_motion (d, ratio, folder0,nRepeat);
end
...
if ( (reRunAll) || (reGenPtnCell) || ~exist(file,'file') )
    ...
    parfor ind = 1:NumCINs
        ...
        [ Timings, V, t ] = MotionSymbolDetector( CIN, ...
...
if ( (reRunAll) || (reSplitDataset) || ~exist(file,'file') )
    [PtnCellTrn, PtnCellTst, indTrn, indTst,indNaN] = MNISTfixedTrainTest(PtnCell,numTrainingImgs);
...
if ( (reRunAll) || (reConvertSpk) || ~exist(file,'file') )
...
%             IsTraining, SimWhichSet, folder0, maxEpoch, lmd,targetRate);
...
if ( (reRunAll) || (reAnalyzeResults) || ~exist(file,'file') )
    [ FinalCorrRate_Trn, FinalCorrRate_Tst, decision_Trn, decision_Tst, freqsCell_Trn, freqsCell_Tst ] = ...
        AnalyzeResults_MajorityVoting_onMNIST(nOutputs, nNeuronPerOutput, folder0);
