function [ Timings, V, t ] = MotionSymbolDetector_withCentroid( CIN, ...
    V0,dt,use_single_exponential,lut1,lut2, THs, label, nGroup, size1, plotfig)
%MotionSymbolDetector_withCentroid 
% Inputs: 
%     CIN, address events
...
% 1st row: time; 2nd row: channel (0~9) not used; 3rd row: label; 4th row: centroid row addr; 5th row: centroid col addr
count = 0;
...
tmp = [tExact; 1*ones(size(tExact)); label*ones(size(tExact)); centroid_atPeak]; 
if ~isempty(tExact),
